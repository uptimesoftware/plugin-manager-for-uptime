...
$plugin_manager_version = "1.0.104";
...
include_once('uptime_lax_functions.inc');
...
// r - auto-restart if necessary
// n - do NOT restart if asked
...
if (strlen(trim($zip_file)) == 0) {
	...
	print "-r - auto-restart if necessary\n";
	print "-n - do NOT restart if asked\n";
...
if ($loglevel == LogLevel::DEBUG) {
	...
	debug_print("Restart? {$auto_restart}");
		...
		if (check_if_plugin_exists($plugin_info, $plugins_dir)) {
			...
			if ($plugin_info->get_restart_core()) {
				...
				elseif ($loglevel != LogLevel::SILENT) {
					...
					elseif ($is_scripted) {
						print_msg(LogLevel::INFO, "-----------------------------------------------------------------------------------------------------");
						print_msg(LogLevel::INFO, "WARNING: Monitor will not work properly until the up.time Data Collector service (core) is restarted!");
						print_msg(LogLevel::INFO, "-----------------------------------------------------------------------------------------------------");
					}
					else {
