...
{if $plugin_restart_httpd == "true"}
<tr class="RowDivider">
	<th>Requires Restart up.time Web Server?</th>
	<td style="color: red;">
		NOTE: User must restart the "up.time Web Server" service manually on the up.time server. Cannot restart the web server while we're using it.<br/>
		<a href="http://docs.uptimesoftware.com/display/UT/Configuring+and+Managing+up.time">You can find the steps on how to restart the "up.time Web Server" here.</a>
	</td>
</tr>
{/if}
{if $plugin_is_update}
