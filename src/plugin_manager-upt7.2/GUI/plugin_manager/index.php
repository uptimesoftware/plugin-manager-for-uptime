...
*/
	...
	if (isset($plugin_info)) {
		...
		$smarty->assign( 'plugin_restart_httpd', $plugin_info->get_restart_httpd_string() );
...
elseif (isset($_REQUEST["install"]) && isset($_REQUEST["uploaded_full_file_name"])) {
	...
	$restart_httpd = "-m";	// default is no restart
	if (isset($_REQUEST["restart_httpd"])) {
		if ($_REQUEST["restart_httpd"] == 'yes') {
			$restart_httpd = '-w';
		}
	}
...
*/
		...
		if ( ! $installed_plugins->pluginExists($plugin_info->get_name()) || $plugin_info->get_sub_type() == PluginSubType::UPDATE) {
			...
			$cmd = "\"{$main_dir}{$local_load_plugin_cmd}\" -l {$LOAD_PLUGIN_LOG_LEVEL} {$restart_core} {$restart_httpd} -f \"{$tmp_target_file}\" 2>&1";
			chdir($main_dir);	// change to the main plugin directory to run the loader script
